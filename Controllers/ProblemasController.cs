    ...
    public class ProblemasController : ControllerBase
    {
        ...
        public IActionResult ObtenerProblemasCompletos()
        {
            ...
            var problemas = from p in dbContext.Problemas
                            ...
                            select new { id = p.IdProblema, p.Nombre, p.Descripcion, categoria = new { c.IdCategoria, c.Nombre }, p.Dificultad, noResueltos = obtenResueltosPorProblema(p.IdProblema), baseDatos = new { b.IdBase, b.Nombre }, p.Solucion, resuelto = compruebaMejorResultadoEnProblema(usuario, p.IdProblema) };
